...
import { periods, stats, monthlyEvents, validTarget, validateRules, validateManual, EARLIEST_DATE, ZONE } from './domain.js';
...
export function handler(action){return async(req,res)=>{
  ...
  try{
    ...
    if(action==='data'){
      if(!method(req,res,['GET']))return;
      const now=new Date(),p=periods(now),db=await database();const [config,c,manual,goals]=await Promise.all([settings(),connection(),db`SELECT id,data FROM fitness_manual`,snapshotGoals(p)]);
      ...
      return json(res,200,{configured:true,authenticated:true,zone:ZONE,periods:p,connected:Boolean(c.refresh_token),lastSync:c.last_sync,error:c.error,rules:config.rules,
        week:stats(events,p.week,p.weekEnd,goals.week,now),month:stats(events,p.month,p.monthEnd,goals.month,now),
        monthEvents:monthlyEvents(events,p,now),
