...
export function monthlyEvents(events, p, now = new Date()) {
  return events.filter(e=>e.date>=EARLIEST_DATE&&e.date>=p.month&&e.date<p.monthEnd)
    .slice().sort((a,b)=>a.start.localeCompare(b.start))
    .map(e=>({...e,completed:Date.parse(e.end)<=now.getTime()}));
}
